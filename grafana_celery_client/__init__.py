...
def includeme(settings):
    ...
    current_app.conf.grafana_celery_client_username = settings.registry.settings.get('grafana_celery_client.username', None)
    current_app.conf.grafana_celery_client_password = settings.registry.settings.get('grafana_celery_client.password', None)
