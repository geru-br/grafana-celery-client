...
class MetricsClientTest(TestCase):
    ...
    def test_send_metric_graphite(self, graphite_metric_mock):
        ...
        send_metric('localhost', 2003, 'test', 10, [{'product': 'consignado'}], timestamp=timestamp,
                    client_type='graphite', environment='dev')
    ...
    def test_send_metric_influx(self, influx_metric_mock):
        timestamp = datetime(2017, 10, 19, 18, 12, 51)
        send_metric('localhost', 8086, 'test', 10, [{'product': 'consignado'}], timestamp=timestamp,
                    client_type='influxdb', environment='dev')
    ...
    def test_send_product_metric_influx(self, influx_metric_mock):
        timestamp = datetime(2017, 10, 19, 18, 12, 51)
        send_product_metric('localhost', 8086, 'consignado', 'test', 10, [{'additional_tag': 'tag'}],
                            timestamp=timestamp, client_type='influxdb', environment='dev')
        influx_metric_mock.assert_called_with('localhost', 'root', 'root', 8086, timestamp, 'dev', 'test', 10,
                                              [{'additional_tag': 'tag'}, {'product': 'consignado'}] )
